(
  ...
  function highlightElement(
    ...
    if (highlightType === "hoverable") {
      overlay.style.border = "3px dashed orange";
    ...
    // Update label for hoverable items
    if (highlightType === "hoverable") {
  ...
  // New helper function to detect hover-sensitive elements
  function isHoverSensitiveElement(element) {
    ...
    const animationName = computedStyle
      ...
      .trim();
    // If there is a non-zero transition or a defined animation,
    // or if the element is already in the hover state, consider it hover-sensitive.
    return (
  ...
  function buildDomTree(node, parentIframe = null) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      // Determine if the element is interactive (clickable) and/or hover-sensitive
      let isInteractive = isInteractiveElement(node);
      let isHoverSensitive = isHoverSensitiveElement(node);
      const computedStyle = window.getComputedStyle(node);
      // If the computed style indicates a pointer cursor or hover-related classes, mark as interactive
      if (
        !isInteractive &&
        (computedStyle.cursor === "pointer" ||
          (node.classList &&
            (node.classList.contains("hover-card") ||
              node.classList.contains("hover-menu"))))
      ) {
        isInteractive = true;
      }
      ...
      nodeData.isHoverSensitive = isHoverSensitive;
      nodeData.isVisible = isVisible;
      nodeData.isTopElement = isTop;

      // If the element is either clickable or hover-sensitive (and visible/top),
      // then highlight it
      if ((isInteractive || isHoverSensitive) && isVisible && isTop) {
        nodeData.highlightIndex = highlightIndex++;
        if (doHighlightElements) {
          let highlightType = isInteractive ? "clickable" : "hoverable";
